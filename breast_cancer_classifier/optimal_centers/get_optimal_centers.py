...
import breast_cancer_classifier.utilities.reading_images as reading_images
import breast_cancer_classifier.data_loading.loading as loading
import breast_cancer_classifier.optimal_centers.calc_optimal_centers as calc_optimal_centers


def fix_center_y_cc(window_location, center):
    center["best_center_y"] = np.mean(np.array([window_location[0], window_location[1]]))
    return center
...
def extract_center(datum, image):
    ...
    if datum["view"] == "CC":
        optimal_center = fix_center_y_cc(
            window_location=datum["window_location"],
...
def load_and_extract_center(datum, data_prefix):
    ...
    """
    filename = os.path.basename(datum["file_path"])
    full_image_path = os.path.join(data_prefix, filename + '.png')
    image = reading_images.read_image_png(full_image_path)
    return filename, extract_center(datum, image)
