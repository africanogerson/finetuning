...
from src.breast_segmentation import ffdmForeground
import breast_cancer_classifier.utilities.saving_images as saving_images
import breast_cancer_classifier.utilities.data_handling as data_handling
...
def crop_mammogram(exam_list_path, cropped_exam_list_path, output_data_folder,
...
def crop_mammogram_one_image(scan, input_file_path, output_file_path, num_iterations, buffer_size):
    ...
    Crops a mammogram, saves as png file, includes the following additional information:
        ...
        - distance_from_starting_side: number of zero columns between the start of the image and start of
           the largest connected component w.r.t. original dicom image.
    """
    image = FFDM(input_file_path).read()
    mask = ffdmForeground(image)
    image *= mask
    image = np.asarray((2**16-1)*image, dtype=np.uint16)
...
def crop_mammogram_one_image_short_path(scan, output_data_folder,
    ...
    """
    short_path = os.path.basename(scan['file_path'])
    full_output_file_path = os.path.join(output_data_folder, short_path + '.png')
    ...
    return list(zip([short_path] * 4, cropping_info))
